...
/**
 * Polygon represents a polygon which can be used to create collisions masks for
 * RuntimeObject.
 *
 * @memberof gdjs
 * @class Polygon
...
gdjs.Polygon.prototype.isConvex = function() {
  ...
  var zProductIsPositive = (this.edges[0][0] * this.edges[0 + 1][1] -
                            this.edges[0][1] * this.edges[0 + 1][0]) > 0;

  for (var i = 1; i < edgesLen - 1; ++i) {
    var zCrossProduct = this.edges[i][0] * this.edges[i + 1][1] -
                        this.edges[i][1] * this.edges[i + 1][0];
    if ((zCrossProduct > 0) !== zProductIsPositive)
      return false;
  }

  var lastZCrossProduct = this.edges[edgesLen - 1][0] * this.edges[0][1] -
 ...
 * Axis Theorem </a>.<br> Based on <a
 * href="http://www.codeproject.com/Articles/15573/2D-Polygon-Collision-Detection">this</a>
 * and <a
 * href="http://stackoverflow.com/questions/5742329/problem-with-collision-response-sat">this</a>
 * article.
 *
 ...
 * @param {boolean | undefined} ignoreTouchingEdges If true, then edges that are
 *     touching each other, without the polygons actually overlapping, won't be
...
gdjs.Polygon.collisionTest = function(p1, p2, ignoreTouchingEdges) {
  ...
  for (var i = 0, len1 = p1.vertices.length, len2 = p2.vertices.length;
       i < len1 + len2; i++) {
    if (i < len1) { // or <=
      edge = p1.edges[i];
    } else {
    ...
    var axis = gdjs.Polygon.collisionTest._statics
                   .axis; // Get the axis to which polygons will be projected
    axis[0] = -edge[1];
    axis[1] = edge[0];
    ...
    var dist =
        gdjs.Polygon.distance(minMaxA[0], minMaxA[1], minMaxB[0], minMaxB[1]);
 ...
 *
 * For some theory, check <a
 * href="https://www.codeproject.com/Tips/862988/Find-the-Intersection-Point-of-Two-Line-Segments">Find
 * the Intersection Point of Two Line Segments</a>.
 *
...
gdjs.Polygon.raycastTest = function(poly, startX, startY, endX, endY) {
  ...
  for (var i = 0; i < poly.edges.length; i++) {
    ...
    // Collinear
    if (Math.abs(crossRS) <= 0.0001 &&
        Math.abs(gdjs.Polygon.crossProduct(deltaQP, r)) <= 0.0001) {
      ...
      var edgeB = gdjs.Polygon.dotProduct(
          axis, [ deltaQP[0] + s[0], deltaQP[1] + s[1] ]);
...
gdjs.Polygon.raycastTest._statics = {
  ...
  return cp;
            ...
            gdjs.Polygon.project =
                function(axis, p, result) {
  ...
  for (var i = 1, len = p.vertices.length; i < len; ++i) {
    ...
    if (dp < result[0])
      ...
      result[1] = dp;
                ...
                gdjs.Polygon.distance =
                    function(minA, maxA, minB, maxB) {
  if (minA < minB)
    return minB - maxA;
  else
    return minA - maxB;
                     ...
                     *
                     * Uses <a
                     * href="https://wrf.ecse.rpi.edu//Research/Short_Notes/pnpoly.html">PNPOLY</a>
  ...
  var inside = false;
  var vi, vj;
  for (var i = 0, j = poly.vertices.length - 1; i < poly.vertices.length;
       j = i++) {
    vi = poly.vertices[i];
    vj = poly.vertices[j];
    if (((vi[1] > y) != (vj[1] > y)) &&
