...
gdjs.Polygon.collisionTest = function (p1, p2, ignoreTouchingEdges) {
    ...
    for (
        var i = 0, len1 = p1.vertices.length, len2 = p2.vertices.length;
        i < len1 + len2;
        i++
    ) {
        if (i < len1) {
        ...
        var dist = gdjs.Polygon.distance(
            minMaxA[0],
            minMaxA[1],
            minMaxB[0],
            minMaxB[1]
...
gdjs.Polygon.raycastTest = function (poly, startX, startY, endX, endY) {
    ...
    for (var i = 0; i < poly.edges.length; i++) {
        ...
        if (
            ...
            var edgeB = gdjs.Polygon.dotProduct(axis, [
                deltaQP[0] + s[0],
                deltaQP[1] + s[1],
            ]);
            // Get overlapping range
            var minOverlap = Math.max(
                Math.min(rayA, rayB),
                Math.min(edgeA, edgeB)
            );
            var maxOverlap = Math.min(
                Math.max(rayA, rayB),
...
gdjs.Polygon.isPointInside = function (poly, x, y) {
    ...
    var vi, vj;
    for (
        var i = 0, j = poly.vertices.length - 1;
