...
Game.prototype.start = function() {
  ...
  var game = this;
  this.intervalId =
      setInterval(function() { GameLoop(game); }, 1000 / this.config.fps);
};
...
Game.prototype.stop = function Stop() { clearInterval(this.intervalId); };
...
WelcomeState.prototype.draw = function(game, dt, ctx) {
  ...
  ctx.fillText("Press 'Space' or touch to start.", game.width / 2,
               game.height / 2);
};
...
GameOverState.prototype.draw = function(game, dt, ctx) {
  ...
  ctx.font = "16px Arial";
  ctx.fillText("Press 'Space' to play again.", game.width / 2,
               game.height / 2 + 40);
};
...
PlayState.prototype.enter = function(game) {
  ...
  var limitLevel = (this.level < this.config.limitLevelIncrease
                        ? this.level
  ...
  this.bombMinVelocity = this.config.bombMinVelocity +
                         (levelMultiplier * this.config.bombMinVelocity);
  this.bombMaxVelocity = this.config.bombMaxVelocity +
  ...
  for (var rank = 0; rank < ranks; rank++) {
    for (var file = 0; file < files; file++) {
      invaders.push(new Invader(
          ...
          (game.gameBounds.top + rank * 20), rank, file, 'Invader'));
    }
  }
...
PlayState.prototype.update = function(game, dt) {
  ...
  for (var i = 0; i < this.invaders.length; i++) {
    ...
    if (!frontRankInvaders[invader.file] ||
...
PlayState.prototype.draw = function(game, dt, ctx) {
  ...
  ctx.fillRect(this.ship.x - (this.ship.width / 2),
               this.ship.y - (this.ship.height / 2), this.ship.width,
               this.ship.height);
  ...
  for (var i = 0; i < this.invaders.length; i++) {
    var invader = this.invaders[i];
    var fillStyle = [
      '#2ac940', '#ff4945', '#FFFF00', '#dad3f2', '#ffdab9', '#0033FF'
    ][Math.floor(invader.y - invader.height / 2) % 6];

    ctx.fillStyle = fillStyle;
    ctx.fillRect(invader.x - invader.width / 2, invader.y - invader.height / 2,
  ...
  for (var i = 0; i < this.bombs.length; i++) {
    // fillStyle = ['#2ac940', '#ff4945', '#cbbeb5', '#dad3f2', '#ffdab9'][i %
    // 5];

    var gradient = ctx.createLinearGradient(0, 0, 170, 0);
    gradient.addColorStop(
        0, [ '#2ac940', '#ff4945', '#cbbeb5', '#dad3f2', '#ffdab9',
             '#f0f8ff' ][Math.floor(Math.random() * 6)]);
    gradient.addColorStop(0.5, [
      '#2ac940', '#ff4945', '#cbbeb5', '#dad3f2', '#ffdab9', '#f0f8ff'
  ...
  //  Draw info.
  var textYpos = game.gameBounds.bottom +
                 ((game.height - game.gameBounds.bottom) / 2) + 14 / 2;
  ctx.font = "14px Arial";
  ctx.fillStyle = '#ffffff';
...
LevelIntroState.prototype.draw = function(game, dt, ctx) {
  ...
  ctx.font = "24px Arial";
  ctx.fillText("Ready in " + this.countdownMessage, game.width / 2,
               game.height / 2 + 36);
  return;
};
...
Sounds.prototype.loadSound = function(name, url) {
  ...
  req.onload = function() {
    self.audioContext.decodeAudioData(
        req.response,
        function(buffer) { self.sounds[name] = {buffer : buffer}; });
  };
  try {
    req.send();
  } catch (e) {
    console.log(
        "An exception occurred getting sound the sound " + name +
        " this might be " +
