...
$("#churn-vs-complexity-graph-container").highcharts({
  ...
  title : {text : "Churn vs Complexity"},
  xAxis : {
    title : {enabled : true, text : "Churn"},
    floor : 0,
    startOnTick : true,
    endOnTick : true
  },
...
function addTitle() {
  ...
  var r = this.renderer, x = this.series[0].center[0] + this.plotLeft,
      y = this.series[0].center[1] + this.plotTop;
  this.title = r.label('<span class="obtained-score">' + score.toFixed(2) +
                           '</span><span class="total-score">/100</span>',
                       0, 0, "", 0, 0, true)
                   .css({color : 'black'})
                   .hide()
                   .add();
...
function getGpaData() {
  ...
  var total =
      ratingACount + ratingBCount + ratingCCount + ratingDCount + ratingFCount;
  return [
    {
      name : 'A',
      y : parseFloat(calculatePercentage(ratingACount, total).toFixed(2)),
      color : COLOR['A']
    },
    {
      name : 'B',
      y : parseFloat(calculatePercentage(ratingBCount, total).toFixed(2)),
      color : COLOR['B']
    },
    {
      name : 'C',
      y : parseFloat(calculatePercentage(ratingCCount, total).toFixed(2)),
      color : COLOR['C']
    },
    {
      name : 'D',
      y : parseFloat(calculatePercentage(ratingDCount, total).toFixed(2)),
      color : COLOR['D']
    },
    {
      name : 'F',
      y : parseFloat(calculatePercentage(ratingFCount, total).toFixed(2)),
      color : COLOR['F']
    },
  ];
};
...
function emphasizeLineFromFragment() { emphasizeLine(window.location.hash) }
...
function scrollToTarget(lineReference) {
  ...
  $.scrollTo(
      lineReference,
      {duration : 300, easing : "linear", offset : {top : -87}, axis : 'y'});
}
...
$("#codeTable").tablesorter({
  // Sort the table
  sortList : [ [ 0, 1 ] ]
});

$("#js-index-table").tablesorter({
  // Sort the table
  sortList : [ [ 0, 0 ] ]
});
...
$(function() {
  $('.table-header').click(function() {
    ...
    if ($(this).hasClass('active')) {
      $(this)
          .find('.sort-type')
          .toggleClass('table-header-asc table-header-desc');
    } else {
...
function assignIdsToCodeLines() {
  $($('.linenums')[1])
      .children()
      .each(function(index) { $(this).attr('id', "L" + index) });
};
...
var initTableFilters = function() {
  $("#codeTable")
      .filterTable({ignoreColumns : [ 2 ], placeholder : 'Filter by Name'});
  $("#js-index-table").filterTable({
    ignoreColumns : [ 2, 3, 4, 5 ],
    placeholder : 'Filter by Smell or Location',
