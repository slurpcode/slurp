 ...
 * false} } });
 *
 * @desc Create a tablesorter interface and disableing the first and second
 * column headers.
 *
 *
...
$.extend({
  tablesorter : new function() {
    ...
    function buildParserCache(table, $headers) {
      ...
      if (rows[0]) {
        ...
        for (var i = 0; i < l; i++) {
          ...
          if ($.metadata &&
              ($($headers[i]).metadata() && $($headers[i]).metadata().sorter)) {
    ...
    function detectParserForColumn(table, rows, rowIndex, cellIndex) {
      var l = parsers.length, node = false, nodeValue = false,
          keepLooking = true;
    ...
    function buildCache(table) {
      ...
      for (var i = 0; i < totalRows; ++i) {
        ...
        for (var j = 0; j < totalCells; ++j) {
          cols.push(
              parsers[j].format(getElementText(table.config, c[0].cells[j]),
                                table, c[0].cells[j]));
        }
    ...
    function appendToTable(table, cache) {
      ...
      var c = cache, r = c.row, n = c.normalized, totalRows = n.length,
          checkCell = (n[0].length - 1), tableBody = $(table.tBodies[0]),
          rows = [];
    ...
    function buildHeaders(table) {
      ...
      $tableHeaders =
          $(table.config.selectorHeaders, table).each(function(index) {
            this.column =
    ...
    function multisort(table, sortList, cache) {
      ...
      for (var i = 0; i < l; i++) {
        ...
        var s = (table.config.parsers[c].type == "text")
                    ? ((order == 0) ? makeSortFunction("text", "asc", c)
                                    : makeSortFunction("text", "desc", c))
                    : ((order == 0) ? makeSortFunction("numeric", "asc", c)
    ...
    this.construct = function(settings) {
      return this.each(function() {
        ...
        $this
            ...
            .bind("applyWidgetId",
                  function(e, id) { getWidgetById(id).format(this); })
...
ts.addParser({
  id : "ipAddress",
  is : function(
      s) { return /^\d{2,3}[\.]\d{2,3}[\.]\d{2,3}[\.]\d{2,3}$/.test(s); },
  format : function(s) {
...
ts.addParser({
  id : "url",
  is : function(s) { return /^(https?|ftp|file):\/\/$/.test(s); },
  format : function(s) {
...
ts.addParser({
  ...
  format : function(
      s) { return $.tablesorter.formatFloat(s.replace(new RegExp(/%/g), "")); },
  type : "numeric"
});
...
ts.addParser({
  ...
  format : function(
      s) { return $.tablesorter.formatFloat(new Date(s).getTime()); },
  type : "numeric"
});
