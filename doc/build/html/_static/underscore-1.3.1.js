...
// are declared here.
var nativeForEach = ArrayProto.forEach, nativeMap = ArrayProto.map,
    nativeReduce = ArrayProto.reduce,
    nativeReduceRight = ArrayProto.reduceRight,
    nativeFilter = ArrayProto.filter, nativeEvery = ArrayProto.every,
    nativeSome = ArrayProto.some, nativeIndexOf = ArrayProto.indexOf,
    nativeLastIndexOf = ArrayProto.lastIndexOf, nativeIsArray = Array.isArray,
    nativeKeys = Object.keys, nativeBind = FuncProto.bind;
...
_.invoke = function(obj, method) {
  ...
  return _.map(obj, function(value) {
    return (_.isFunction(method) ? method || value : value[method])
        .apply(value, args);
  });
};
...
_.pluck = function(
    obj, key) { return _.map(obj, function(value) { return value[key]; }); };
...
_.max = function(obj, iterator, context) {
  ...
  each(obj, function(value, index, list) {
    var computed =
        iterator ? iterator.call(context, value, index, list) : value;
...
_.rest = _.tail = function(
    array, index,
    guard) { return slice.call(array, (index == null) || guard ? 1 : index); };

// Trim out all falsy values from an array.
_.compact = function(
    array) { return _.filter(array, function(value) { return !!value; }); };
...
_.without = function(
    array) { return _.difference(array, slice.call(arguments, 1)); };
...
_.intersection = _.intersect = function(array) {
  ...
  return _.filter(_.uniq(array), function(item) {
    return _.every(rest,
                   function(other) { return _.indexOf(other, item) >= 0; });
  });
};
...
_.memoize = function(func, hasher) {
  ...
  return function() {
    ...
    return _.has(memo, key) ? memo[key]
...
function eq(a, b, stack) {
  ...
  case '[object Boolean]':
    ...
    // by their millisecond representations. Note that invalid dates with
    // millisecond representations of `NaN` are not equivalent.
    return +a == +b;
...
_.isArguments = function(
    obj) { return toString.call(obj) == '[object Arguments]'; };
...
_.isFunction = function(
    obj) { return toString.call(obj) == '[object Function]'; };
...
_.mixin = function(obj) {
  each(_.functions(obj),
       function(name) { addToWrapper(name, _[name] = obj[name]); });
};
...
var unescape = function(
    code) { return code.replace(/\\\\/g, '\\').replace(/\\'/g, "'"); };
  ...
  var tmpl =
      ...
      str.replace(/\\/g, '\\\\')
          ...
          .replace(
              c.escape || noMatch,
              function(match,
                       code) { return "',_.escape(" + unescape(code) + "),'"; })
          .replace(
              c.interpolate || noMatch,
              function(match, code) { return "'," + unescape(code) + ",'"; })
          .replace(c.evaluate || noMatch,
                   function(match, code) {
