...
// - 9.1 throw exceptions when non-strict code (e.g., ASP.NET 4.5) accesses
// strict mode arguments.callee.caller (trac-13335). But as of jQuery 3.0
// (2016), strict mode should be common enough that all such attempts are
// guarded in a try block.
"use strict";
...
var isWindow = function isWindow(
    obj) { return obj != null && obj === obj.window; };
...
jQuery.extend({
  ...
  globalEval : function(
      code, options) { DOMEval(code, {nonce : options && options.nonce}); },
...
var Sizzle =
    ...
    (function(window) {
        ...
        if (!seed) {
          ...
          if (documentIsHTML) {
            ...
            if (support.qsa && !nonnativeSelectorCache[selector + " "] &&
              ...
              // child or descendant combinators, which is not what we want. In
              // such cases, we work around the behavior by prefixing every
              // selector in the list with an ID selector referencing the scope
              // context. Thanks to Andrew Dupont for this technique.
      ...
      function createDisabledPseudo(disabled) {
        ...
        return function(elem) {
          ...
          if ("form" in elem) {
            ...
            // legend, menu, track), but it shouldn't even exist on them, let
            // alone have a boolean value.
      ...
      isXML = Sizzle.isXML = function(elem) {
        ...
        // loading iframes https://bugs.jquery.com/ticket/4833
      ...
      setDocument = Sizzle.setDocument = function(node) {
        ...
        // errors (jQuery #13936)
        if (preferredDoc !== document && (subWindow = document.defaultView) &&
      ...
      */
        ...
        // error See https://bugs.jquery.com/ticket/13378
        rbuggyQSA = [];
      ...
      */
      ...
      Sizzle.escape = function(
          sel) { return (sel + "").replace(rcssescape, fcssescape); };
      ...
      if (!assert(function(
              el) { return el.getAttribute("disabled") == null; })) {
...
function winnow(elements, qualifier, not) {
  if (isFunction(qualifier)) {
    return jQuery.grep(
        elements,
        function(elem, i) { return !!qualifier.call(elem, i, elem) !== not; });
  }
  ...
  if (typeof qualifier !== "string") {
    return jQuery.grep(
        elements,
        function(
            elem) { return (indexOf.call(qualifier, elem) > -1) !== not; });
  }
...
jQuery.filter = function(expr, elems, not) {
  ...
  return jQuery.find.matches(
      expr, jQuery.grep(elems, function(elem) { return elem.nodeType === 1; }));
};
            ...
            return jQuery
                .Deferred(function(newDefer) {
                  jQuery.each(tuples, function(i, tuple) {
                    // Map tuples (progress, done, fail) to arguments (done,
                    // fail, progress)
                    var fn = isFunction(fns[tuple[4]]) && fns[tuple[4]];

                    // deferred.progress(function() { bind to newDefer or
                    // newDefer.notify }) deferred.done(function() { bind to
                    ...
                    deferred[tuple[1]](function() {
                      ...
                      if (returned && isFunction(returned.promise)) {
                        ...
                        newDefer[tuple[0] + "With"](this, fn ? [ returned ]
                                                             : arguments);
                      }
                    });
                  });
            ...
            function resolve(depth, deferred, handler, special) {
              return function() {
                var that = this, args = arguments,
                    ...
                    // Only normal processors (resolve) catch and reject
                    // exceptions
                ...
                if (depth) {
                  ...
                  // Call an optional hook to record the stack, in case of
                  // exception since it's otherwise lost when execution goes
                  // async
...
jQuery.readyException = function(
    error) { window.setTimeout(function() { throw error; }); };
...
jQuery.fn.ready = function(fn) {
  readyList
      ...
      // registration.
      .catch(function(error) { jQuery.readyException(error); });
...
var isAttached = function(
        elem) { return jQuery.contains(elem.ownerDocument, elem); },
...
var isHiddenWithinTree = function(elem, el) {
  ...
  return elem.style.display === "none" ||
         elem.style.display === "" &&
             ...
             // confirm that elem is in the document.
             isAttached(elem) &&
...
function adjustCSS(elem, prop, valueParts, tween) {
  ...
  if (initialInUnit && initialInUnit[3] !== unit) {
    ...
    // Support: Firefox <=54
    // Halve the iteration target value to prevent interference from CSS upper
    // bounds (gh-2144)
    initial = initial / 2;
    ...
    while (maxIterations--) {
      ...
      // Finish if the scale equals or crosses 1 (making the old*new product
      // non-positive).
...
function showHide(elements, show) {
  ...
  for (; index < length; index++) {
    ...
    if (show) {
      ...
      // nonempty display value (either inline or about-to-be-restored)
      if (display === "none") {
...
jQuery.event = {
  ...
  dispatch : function(nativeEvent) {
    ...
    while ((matched = handlerQueue[i++]) && !event.isPropagationStopped()) {
      ...
      while ((handleObj = matched.handlers[j++]) &&
        ...
        // If the event is namespaced, then each handler is only invoked if it
        // is specially universal or its namespaces are a superset of the
        // event's.
  ...
  handlers : function(event, handlers) {
    var i, handleObj, sel, matchedHandlers, matchedSelectors,
        handlerQueue = [], delegateCount = handlers.delegateCount,
        cur = event.target;
  ...
  special : {
    ...
    click : {
      ...
      setup : function(data) {
        // For mutual compressibility with _default, replace `this` access with
        // a local var.
      ...
      // Also prevent it if we're currently inside a leveraged native-event
      // stack
      _default : function(event) {
        var target = event.target;
        return rcheckableType.test(target.type) && target.click &&
                   nodeName(target, "input") && dataPriv.get(target, "click") ||
               nodeName(target, "a");
      }
    },
  ...
  jQuery.event.add(el, type, {
    ...
    handler : function(event) {
      ...
      if ((event.isTrigger & 1) && this[type]) {
        ...
        // capture object from an async native handler (gh-4350)
        if (!saved.length) {

          // Store arguments for use when handling the inner native event
          // There will always be at least one argument (an event object), so
          // this array will not be confused with a leftover capture object.
          ...
          // propagated from triggering the native event and prevent that from
          // happening again here. This technically gets the ordering wrong
          // w.r.t. to `.trigger()` (in which the bubbling surrogate propagates
          // *after* the non-bubbling base), but that seems less bad than
          // duplication.
        ...
        dataPriv.set(this, type, {
          value : jQuery.event.trigger(
              ...
              jQuery.extend(saved[0], jQuery.Event.prototype), saved.slice(1),
              this)
        });
...
jQuery.each(
    {focus : "focusin", blur : "focusout"}, function(type, delegateType) {
...
function domManip(collection, args, callback, ignored) {
  ...
  if (valueIsFunction || (l > 1 && typeof value === "string" &&
...
function boxModelAdjustment(elem, dimension, box, isBorderBox, styles,
  ...
  for (; i < 4; i += 2) {
    ...
    if (!isBorderBox) {
      ...
      // seeking "content" or "padding" or "margin"
    } else {
  ...
  if (!isBorderBox && computedVal >= 0) {
    ...
    // gutter, and border Assuming integer scroll gutter, subtract the rest and
    // round down
    delta += Math.max(0, Math.ceil(
                             elem["offset" + dimension[0].toUpperCase() +
                                  dimension.slice(1)] -
                             computedVal - delta - extra - 0.5

                             // If offsetWidth/offsetHeight is unknown, then we
                             // can't determine content-box scroll gutter Use an
                             // explicit zero to avoid NaN (gh-3964)
                             )) ||
             0;
  }
...
function getWidthOrHeight(elem, dimension, extra) {
  ...
  var styles = getStyles(elem),
      ...
      // To avoid forcing a reflow, only fetch boxSizing if we need it
      // (gh-4322). Fake content-box until we know it's needed to know the true
      // value.
      boxSizingNeeded = !support.boxSizingReliable() || extra,
      isBorderBox = boxSizingNeeded && jQuery.css(elem, "boxSizing", false,
                                                  styles) === "border-box",
  ...
  // Also use offsetWidth/offsetHeight for misreported inline dimensions
  ...
  // hidden/disconnected. In those cases, the computed value can be trusted to
  // be border-box
  if ((!support.boxSizingReliable() && isBorderBox || val === "auto" ||
    ...
    // Where available, offsetWidth/offsetHeight approximate border box
    // dimensions. Where not available (e.g., SVG), assume unreliable box-sizing
    // and interpret the retrieved value as a content box dimension.
  ...
  return (val +
          boxModelAdjustment(elem, dimension,
                             extra || (isBorderBox ? "border" : "content"),
                             valueIsBorderBox, styles,

                             // Provide the current computed size to request
                             // scroll gutter calculation (gh-3589)
                             val)) +
         "px";
}
...
jQuery.extend({
  ...
  style : function(elem, name, value, extra) {
    ...
    if (value !== undefined) {
      ...
      // properties) The isCustomProp check can be removed in jQuery 4.0 when we
      // only auto-append "px" to a few hardcoded values.
...
jQuery.each([ "height", "width" ], function(i, dimension) {
  jQuery.cssHooks[dimension] = {
    get : function(elem, computed, extra) {
      if (computed) {
        ...
        return rdisplayswap.test(jQuery.css(elem, "display")) &&
                       ...
                       // zero getBoundingClientRect().width unless display is
                       // changed. Support: IE <=11 only Running
                       // getBoundingClientRect on a disconnected node in IE
                       // throws an error.
                       (!elem.getClientRects().length ||
    ...
    set : function(elem, value, extra) {
      ...
      // computed and faking a content-box to get border and padding (gh-3699)
      if (isBorderBox && scrollboxSizeBuggy) {
        subtract -= Math.ceil(
            elem["offset" + dimension[0].toUpperCase() + dimension.slice(1)] -
            parseFloat(styles[dimension]) -
            boxModelAdjustment(elem, dimension, "border", false, styles) - 0.5);
      }
...
function defaultPrefilter(elem, props, opts) {
  var prop, value, toggle, hooks, oldfire, propTween, restoreDisplay, display,
      isBox = "width" in props || "height" in props, anim = this, orig = {},
      style = elem.style, hidden = elem.nodeType && isHiddenWithinTree(elem),
  ...
  if (isBox && elem.nodeType === 1) {
    ...
    if (display === "inline" ||
      ...
      if (jQuery.css(elem, "float") === "none") {
        ...
        // Restore the original display value at the end of pure show/hide
        // animations
        if (!propTween) {
...
jQuery.fn.extend({
  ...
  hasClass : function(selector) {
    ...
    className = " " + selector + " ";
    while ((elem = this[i++])) {
      if (elem.nodeType === 1 && (" " + stripAndCollapse(getClass(elem)) +
                                  " ").indexOf(className) > -1) {
        return true;
      }
    }
...
jQuery.param = function(a, traditional) {
  var prefix, s = [], add = function(key, valueOrFunction) {
    ...
    var value =
        isFunction(valueOrFunction) ? valueOrFunction() : valueOrFunction;
...
jQuery.extend({
  ...
  ajax : function(url, options) {
    ...
    var transport,
        ...
        jqXHR = {
          ...
          getAllResponseHeaders :
              function() { return completed ? responseHeadersString : null; },
    ...
    s.dataTypes =
        (s.dataType || "*").toLowerCase().match(rnothtmlwhite) || [ "" ];
...
jQuery._evalUrl = function(url, options) {
  return jQuery.ajax({
    ...
    dataFilter : function(response) { jQuery.globalEval(response, options); }
  });
};
...
jQuery.expr.pseudos.hidden = function(
    elem) { return !jQuery.expr.pseudos.visible(elem); };
...
jQuery.fn.extend({
  ...
  offset : function(options) {
    ...
    // (gh-2310) Support: IE <=11 only Running getBoundingClientRect on a
  ...
  // position() relates an element's margin box to its offset parent's padding
  // box
  ...
  position : function() {
    ...
    // position:fixed elements are offset from the viewport, which itself always
    // has zero offset
...
jQuery.isNumeric = function(obj) {
  ...
  return (type === "number" || type === "string") &&
         ...
         // literals ("0x...") subtraction forces infinities to NaN
